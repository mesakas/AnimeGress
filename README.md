# AnimeGrass

AnimeGrass 是面向 Unity URP 的风格化草系统。它提供手工铺设、单株编辑、GPU Instancing、LOD 点状渐隐、全局风场、风场颜色变化、阴影和视锥剔除，并包含地表属性缓存、三维排除/推动 Volume 与远景颜色及伪阴影覆盖。

当前包版本为 `1.0.0`，Git 发布标签为 `v1.0`。

## 效果预览

![AnimeGrass 风格化草场效果](Documentation~/demo.gif)

![AnimeGrass 草场铺设工具与配置界面](Documentation~/animegrass-editor-tools.png)

## 系统组成

- `AnimeGrassField`：保存确定性的草实例数据，负责分块、LOD、视锥剔除和实例化绘制。
- `AnimeGrassPrototype`：定义草类型、模型校正、风权重和各级 LOD。
- `AnimeGrassRendererFeature`：在每个 URP 摄像机渲染阶段提交草绘制。
- `AnimeGrassFarField`：在最后一级 LOD 之外用地表覆盖保留草场颜色、风色和伪阴影变化。
- `AnimeGrassWindZone`：分别设置草叶摆动、草叶阵风和独立的风色波纹变化。
- `AnimeGrassPainterWindow`：提供普通鼠标、铺设、删除和单株编辑工作流。
- `AnimeGrassInstanced.shader`：提供风格化颜色、风动、点状渐隐和阴影处理。
- `AnimeSurfaceCache`：捕获固定世界范围内的地表颜色、世界法线、高度和排除草遮罩。
- `AnimeSurfaceCacheSource`：覆盖特殊材质的捕获贴图、颜色和排除草遮罩。
- `grassVolume`：负责在球形或盒形三维范围内排除草和实时推动草叶。

## 环境要求

- Unity 6.0 或更高版本
- Universal Render Pipeline 17 或兼容版本
- 当前版本使用 URP Compatibility Mode 的 `ScriptableRenderPass.Execute`
- 草材质需要启用 GPU Instancing

## 安装

本项目已经以嵌入式包方式安装在 `Packages/com.ming.animegrass`。

可以在 Package Manager 中使用 Git URL 安装，并在 URL 末尾指定 `#v1.0`。也可以选择 **Add package from disk**，然后选择本目录的 `package.json`。

## URP 配置

打开当前使用的 URP Renderer Data，在 Renderer Features 中添加 `AnimeGrassRendererFeature`。所有需要显示草的 Renderer Data 都应添加该 Feature。

当前 `v1.0` 使用 Compatibility Mode。请在 `Project Settings > Graphics > URP` 中启用 Compatibility Mode。后续版本可以增加 Render Graph 路径。

## 双端质量档位

`AnimeGrassRendererFeature` 提供 `自动（按构建平台）`、`桌面端完整效果` 和 `移动端优化` 三种质量档位。推荐双端项目保持 `自动`：Android/iOS 使用移动端路径，Windows、macOS 和 Linux 使用桌面端路径。在编辑器中切换活动构建平台后，自动档也会使用对应路径；也可以临时强制移动端档在 PC 上检查降级效果。

桌面端档始终保留地表属性缓存、实时 GrassVolume、主光源阴影、远景覆盖和动态明暗图案，不受“移动端保留功能”选项影响。移动端默认只保留“远景草覆盖”，其余高成本功能通过 Shader 变体从对应阶段中编译移除，而不是仅在运行时使用分支跳过。

移动端可按项目需要恢复以下功能：

- `实体草地表缓存响应`：恢复地表颜色、法线、湿润/积雪/烧焦和静态排除遮罩。会增加实体草片元纹理采样与插值数据。
- `实时 GrassVolume 交互`：恢复实时排除和推动。实体草顶点与远景覆盖像素最多遍历 16 个 Volume，移动端建议只在确实需要时启用，并控制活动 Volume 数量。
- `主光源实时阴影`：恢复实体草和远景覆盖的主光源阴影图采样。草自身是否投射阴影仍由各 LOD 配置控制。
- `远景草覆盖`：保留最后实体 LOD 之外的低成本地表覆盖。关闭后 Renderer Feature 不再请求该功能所需的深度与法线输入。
- `远景动态明暗图案`：恢复远景覆盖的多层噪声、风向波纹和伪阴影动画。它是全屏片元路径，通常应当是移动端最后恢复的效果。

内置 `AnimeGrass/Anime Grass Instanced` 和远景覆盖 Shader 已支持这些裁剪关键字。自定义草 Shader 不会自动获得同样的指令级裁剪，需要自行声明并处理对应的 `ENLYN_GRASS_DISABLE_*` 关键字。

## 性能建议

运行时会缓存单株基础矩阵、在进入单株循环前执行分块距离剔除，并直接计算当前及过渡中的最多两个 LOD。Renderer Feature 也只会为同一摄像机绑定一次共享状态。这些优化同时作用于桌面端和移动端，但当前实体草仍使用每批最多 1023 株的 `DrawMeshInstanced`，不是 Compute Shader 驱动的间接绘制。

移动端建议从以下配置开始，再根据真机 Profiler 调整：

- 质量档位使用 `自动`，移动端保留功能先只勾选 `远景草覆盖`。
- 近处使用低顶点 Mesh，远处降低“草数量比例”，最后实体 LOD 设置有限结束距离并交给远景覆盖。
- 避免将任意实体 LOD 的结束距离设为 `0`；有限距离可以让整个不可见分块在单株处理前被剔除。
- 地表属性缓存使用 `变化时更新` 或 `仅手动更新`；移动端避免每帧重建高分辨率缓存。
- 同时活动的实时 GrassVolume 越少越好。必须恢复交互时，优先只保留角色附近的少量 Volume。
- 分别在目标 Android/iOS 真机和最低配置 PC 上检查 CPU 主线程、GPU 帧时间、批次数和实际提交实例数，不以编辑器帧率替代真机结论。

## 快速开始

1. 在 URP Renderer Data 的 Renderer Features 中添加 `AnimeGrassRendererFeature`。
2. 使用 `GameObject > AnimeGrass > 二次元草场` 创建草场。
3. 使用 `Assets > Create > AnimeGrass > 草类型配置` 创建草类型。
4. 为每个 LOD 指定 Mesh、Material、显示距离和渐隐距离。
5. 使用 `Window > AnimeGrass > 草场铺设工具` 铺设、删除或编辑单株草。
6. 在场景中添加 `AnimeGrassWindZone`，分别控制全局风向、草叶摆动和风色波纹。

`游戏 LOD 参考摄像机` 可以留空。留空时每个摄像机使用自身位置计算 LOD；指定后只影响游戏摄像机的 LOD 距离，不限制 Scene 视图或其他摄像机显示。

## 草材质贴图

`AnimeGrass/Anime Grass Instanced` 同时支持基础贴图自带 Alpha 和独立 Alpha 贴图。不需要基础贴图时，将“基础贴图”留空，只指定“独立 Alpha 贴图”；草叶颜色会继续由基础颜色、根部颜色、顶部颜色和实例颜色生成。

- `Alpha 贴图通道 = R`：读取灰度图的红色通道，适合黑白遮罩。建议在贴图导入设置中关闭 `sRGB (Color Texture)`。
- `Alpha 贴图通道 = Alpha`：读取 PNG/TGA 等贴图自带的透明通道。
- `透明裁剪阈值`：值越大，被移除的低 Alpha 区域越多。

基础贴图和独立 Alpha 贴图有各自的 Tiling/Offset。同时指定两张贴图时，基础贴图的 Alpha 与独立遮罩相乘，便于进一步修剪草叶轮廓。

## LOD 配置

每级 LOD 包含：

- Mesh 与 Material
- Sub Mesh Index
- 开始距离和结束距离
- 点状渐隐距离
- 是否始终面向观察目标
- 投射阴影与接收阴影开关

最后一级 LOD 的结束距离就是草的最大显示距离。`结束距离 = 0` 表示没有上限，不建议对大量实例使用。

“距离计算模式”支持四种规则：三维直线距离；世界 XY 平面距离与世界 Z 轴距离分别计算并取更远级别；完全忽略 Z 的“仅 XY 距离”；以及完全忽略 Unity 高度 Y 的“仅水平 XZ 距离”。常规 Unity 地面位于 XZ 平面，高空俯视时应使用“仅水平 XZ 距离”。

启用“远景草替换后续 LOD”后，可以指定“最后实体 LOD”。该索引之后的 LOD 配置会保留但不再提交绘制，最后实体 LOD 按自身结束距离渐隐。例如最后实体 LOD 设为 `0`，就可以跳过后续面片草，直接交给远景草覆盖。

第一级 LOD 的开始距离固定为 `0`，后续级会自动连接到上一级的结束距离。从 LOD N 切换到 LOD N+1 时，两级会在 LOD N 的“渐隐距离”内同时提交。旧级使用正向随机点阵逐步削减，新级使用互补点阵填充被移除的位置，不使用半透明混合。`渐隐距离 = 0` 时仍会立即切换。运行时和 Scene 编辑预览使用相同规则。

自定义草 Shader 需要支持有符号的 `_InstanceFade`：正值使用正向点阵，负值使用反向点阵。使用内置 `AnimeGrass/Anime Grass Instanced` 时无需额外处理。

“始终面向观察目标”按 LOD 单独配置，适合远距离的单面片或少量交叉面片草。启用后，实例会绕各自的草根法线旋转，因此斜坡上的草仍保持贴合地表。完整模型草通常保持关闭。

草场组件的“草面片朝向目标”可以指定玩家角色或其他 Transform；为空时使用当前渲染相机。Game 视图可以面向角色目标，Scene 视图始终使用自己的 Scene 相机，以便从任意编辑视角正确查看面片。朝向在 CPU 构建实例矩阵时计算，不要求材质使用 AnimeGrass Shader。

每个草类型还提供独立的“距离密度”配置。LOD 决定使用哪个 Mesh，距离密度决定在对应距离实际保留多少株草：

- `近处草数量比例`：密度过渡开始距离以内保留的实例比例，通常使用 `1`。
- `远处草数量比例`：密度过渡结束距离以外保留的实例比例，通常使用 `0.2-0.5`。
- `密度过渡开始/结束距离`：在该区间内从近处比例平滑过渡到远处比例。
- `单株切换柔和度`：控制草株跨过确定性随机阈值时的点状渐隐，通常使用 `0.05-0.12`。
- `密度随机种子`：只改变被保留草株的稳定分布，不修改已铺设实例。

距离密度在 CPU 提交实例前生效，因此远处比例降低后能够实际减少 DrawMeshInstanced 中的实例数量。处于切换边缘的少量草株会继续提交，并使用现有点状剔除完成渐隐。

## 远景草覆盖

`AnimeGrassFarField` 用于在真实草被最后一级 LOD 剔除后，继续保留草场的整体颜色、风场颜色和动态明暗。它根据草场内已经序列化的实例生成颜色/覆盖率和草根高度缓存，再通过摄像机深度把结果叠加到原来的地表上。该方案不绘制远距离草 Mesh，也不生成真实草阴影。

### 启用方法

1. 选中带有 `AnimeGrassField` 的草场对象。
2. 在草场 Inspector 的“远景覆盖（可选）”中点击“添加远景草覆盖”。
3. 点击“匹配最后一级 LOD 渐隐距离”，或手动让覆盖过渡与最后一级 LOD 的渐隐距离重合。
4. 根据铺草间距调整“单株覆盖半径”，然后点击“立即重建覆盖缓存”。
5. 确认当前 URP Renderer Data 已添加 `AnimeGrassRendererFeature`。Feature 会自动请求相机深度纹理。

远景覆盖应选择与草类型一致的距离计算模式。草类型使用“仅水平 XZ 距离”时，远景覆盖也选择该模式；这样相机沿世界 Y 轴升高不会提前触发远景覆盖，真实草与远景草会按相同的 XZ 距离完成交接。

推荐先使用以下范围：

- 覆盖开始距离：`最后一级 LOD 结束距离 - 渐隐距离`。
- 覆盖完全显示距离：最后一级 LOD 的结束距离。
- 最远渐隐开始距离：从该距离开始逐渐减少远景覆盖点，通常根据关卡能见度设置。
- 最大显示距离：远景颜色、风色和伪阴影完全消失的距离，必须大于最远渐隐开始距离。
- 单株覆盖半径：铺草间距的 `0.5-0.8` 倍。
- 覆盖轮廓柔和度：通常使用 `0.45-0.75`；数值越大，草场外轮廓和小孔边缘的点状过渡越宽。
- 覆盖颜色强度：`0.5-0.8`。
- 内部孔洞填充：通常使用 `2-4` 像素；只处理株间小孔，不用于填充道路等大面积空白。
- 点状渐隐颗粒大小：推荐 `1`，需要更明显的颗粒时使用 `2`。
- 地表过滤边缘柔和度：通常使用 `0.5-0.8`；用于柔化高度容差和斜坡法线产生的斜向边界。
- 伪阴影强度：`0.1-0.25`。
- 伪阴影扰动：`0.4-0.7`，扰动斑块尺寸通常使用 `4-10` 米。
- 波纹弧度：`0.6-0.9`；`0` 为平行直线波，`1` 为横向弯曲更明显的定向波峰。
- 波纹间距：通常使用 `6-12` 米；数值越小，同一范围内出现的波纹越多。
- 波纹弯曲尺度：通常使用 `12-30` 米；数值越大弯曲越宽缓，数值越小弯曲越频繁。
- 缓存分辨率：普通草场使用 `256`，大范围或需要清晰边界时使用 `512`。

覆盖进入距离必须与最后一级 LOD 的渐隐区间重叠，否则两者之间会出现没有真实草、也没有远景覆盖的空带。例如最后一级 LOD 在 `30` 米结束、渐隐距离为 `3` 米时，建议将覆盖开始/完全显示距离设为 `27 / 30`，不要设为 `57 / 60`。可直接点击“匹配最后一级 LOD 渐隐距离”自动填写。远景覆盖在“最远渐隐开始距离”到“最大显示距离”之间再次使用点状剔除逐渐隐出，超过最大显示距离后不再写入颜色。

“地表高度容差”用于区分上下楼层和悬空遮挡物；只会覆盖与草根缓存高度接近的可见表面。“最低向上法线”用于排除墙面和陡峭侧面。“地表过滤边缘柔和度”同时扩大这两个过滤条件的渐变区，避免低多边形斜坡或高度交界沿三角面形成硬切线。修改草实例、草类型材质或远景覆盖参数后，缓存会标记为待重建，并在下一次渲染时更新，也可以手动立即重建。

远景覆盖颜色从草类型第一个有效 LOD 材质的根部/顶部颜色、实例颜色和颜色倍率计算。它只表达远处的草场体块，不保留单株轮廓、草叶实时投影或草叶遮挡关系。

开启“匹配近景草颜色”后，远景覆盖会自动使用草材质的代表色、阴影色、接收阴影强度和风色响应，并在完整覆盖区使用完整颜色强度。远景 Shader 会根据可见地表法线应用与近景草相同的主光源/阴影色近似，从而降低 LOD 交接时的色差。关闭后可手动调整覆盖颜色倍率、点保留比例和风色响应。由于远景覆盖是连续地表颜色，而近景草由离散草叶和叶片间隙组成，复杂基础贴图、不同草叶法线和屏幕覆盖率仍可能造成轻微感知差异。

远景覆盖不会使用 `SrcAlpha` 半透明混合。缓存中的连续覆盖率、距离渐隐、高度匹配和坡度过滤结果会转换为稳定的屏幕空间随机点状剔除：覆盖率越低，保留的不透明颜色点越少，地面会从点之间逐渐显露。这样可以得到柔和的密度过渡，同时避免半透明叠加带来的排序和混合开销。“覆盖轮廓柔和度”控制缓存外轮廓，“地表过滤边缘柔和度”控制投射到斜坡后的过滤边界。“内部孔洞填充”会在缓存重建时闭合株间小孔，修改后需要点击“立即重建覆盖缓存”；“点状渐隐颗粒大小”只控制最终点的尺寸。

伪阴影始终沿全局风向单向传播。系统通过横向相位偏移让每条波峰产生宽缓弯曲，再混合次级定向波和多方向世界空间扰动来打散规则直线，不会生成以某一点向外扩散的圆形波纹，也不需要额外噪声纹理采样。“扰动斑块尺寸”控制明暗区域大小，“扰动移动速度”控制图案沿全局风向漂移的速度。扰动只作用于伪阴影，不会改变草场覆盖边界。

## 铺设规则

铺设工具只在当前选中的 `AnimeGrassField` 上工作。目标表面被锁定后，射线只接受该 Collider，因此上层遮挡物不会抢占下层表面的铺设结果。

场景数据直接序列化在草场组件中，铺设结果是确定性的。单株编辑模式可以使用 Unity 默认移动、旋转和缩放工具修改实例。

`随机缩放比例 XYZ` 分别控制三个局部轴的尺寸变化。数值 `0.15` 表示对应轴在 `85%` 到 `115%` 之间随机，`0` 表示保持原始比例。`随机旋转比例 XYZ` 分别控制三个局部轴的旋转范围，数值 `1` 表示 `-180°` 到 `180°`，`0.5` 表示 `-90°` 到 `90°`。默认只启用 Y 轴随机旋转。随机结果会在铺设时写入实例并随场景保存，不会在运行时继续变化。

## 模型规范

- 推荐模型根部位于局部坐标原点。
- 推荐局部 Y 轴向上、Z 轴向前。
- Blender 导出前应用 Rotation 和 Scale。
- 模型轴向或单位不一致时，在草类型配置中使用位置、旋转和缩放校正。
- 面片草与完整模型草都可以作为任意 LOD Mesh。

## 用户资源

草模型、材质、草类型配置和场景属于项目内容，不属于插件代码。它们可以放在任意 `Assets` 子目录。当前项目的演示内容保留在 `Assets/Enlyn/grass/Demo`。

## API 兼容性

为保持已有项目兼容，公开类型继续使用 `Enlyn.Grass` 命名空间。包程序集名称为：

- `Ming.AnimeGrass.Runtime`
- `Ming.AnimeGrass.Editor`

## 地表属性缓存

地表属性缓存是完整 RVT 之前的轻量方案。它使用固定大小的世界空间正交缓存，不依赖某一台游戏摄像机，也不需要额外的 Renderer Feature。缓存由三张纹理组成：

- 颜色纹理：保存地表基础颜色与有效区域。
- 数据纹理：RGB 保存编码后的世界法线，A 保存捕获高度范围内的归一化高度。
- 遮罩纹理：A 保存排除草强度，RGB 保留为零。

### 创建和配置

1. 使用 `GameObject > AnimeGrass > 地表属性缓存` 创建缓存对象。
2. 将对象放到需要覆盖区域的中心，设置 `世界范围 XZ`、`垂直捕获高度` 和 `缓存分辨率`。
3. 设置 `地表 Layer`。缓存只捕获这些 Layer 上的 `MeshRenderer`、`SkinnedMeshRenderer` 和 Unity `Terrain`。
4. 点击 `立即重建缓存`，在 Inspector 的三张缓存预览中检查结果。
5. 在草材质中设置地表颜色、地表法线和排除遮罩影响。地表颜色会在草根处直接向缓存颜色混合。

缓存从上向下捕获，并使用深度保留同一 XZ 位置上最高的表面。Unity Terrain 支持高度、法线以及最多 8 个 Terrain Layer 的颜色混合。Mesh 材质会自动读取 `_BaseMap` 或 `_MainTex`，以及 `_BaseColor` 或 `_Color`；使用自定义属性名时，在目标物体或父物体上添加 `AnimeSurfaceCacheSource` 并覆盖基础贴图和颜色。

### 缓存范围与质量

缓存范围是以 `AnimeSurfaceCache` 对象 Transform 为中心的三维捕获盒。“世界范围 XZ”决定地面覆盖宽度和长度，“垂直捕获高度”以组件 Y 坐标为中心向上、向下各覆盖一半。比如 `128 x 128` 米、垂直高度 `80` 米会覆盖中心左右/前后各 `64` 米、上下各 `40` 米。

缓存纹理是正方形，因此每个轴的世界纹素尺寸分别为 `世界范围 X / 分辨率` 和 `世界范围 Z / 分辨率`。`128 x 128` 米配合 `1024` 分辨率约为 `0.125` 米/像素。扩大范围而不提高分辨率会降低地表颜色、法线和排除边界的空间精度；分辨率翻倍时，纹理显存和重建像素成本约增加到四倍。

开启“显示缓存范围”后，选中缓存对象会在 Scene 视图中显示可穿透地形和草叶的三维捕获盒、中心 XZ 覆盖面，以及尺寸、Y 高度范围和 X/Z 米每像素标签。开启“未选中时也显示”可让取消选择后的范围继续显示。跟随目标只改变缓存中心的 XZ 位置，Y 高度中心仍由缓存对象自身控制。

### 更新模式

- `变化时更新`：缓存对象、`AnimeSurfaceCacheSource`、`grassVolume` 或编辑器层级/资源发生变化时更新，适合静态场景。
- `定时更新`：按指定秒数更新，也会立即响应已登记的变化，适合少量动态地表。
- `每帧更新`：每帧完整重建，成本最高，只适合较小缓存或必须实时变化的场景。
- `仅手动更新`：只通过 Inspector 按钮或 `RefreshNow()` 更新，适合完全静态的确定性场景。

普通动态 Renderer 的移动不会自动触发 `变化时更新`。这类表面应添加 `AnimeSurfaceCacheSource`，或者使用定时/每帧更新。运行时代码修改 Terrain、材质或纹理后，也可以调用 `MarkDirty()`、`MarkDirty(Bounds)` 或静态方法 `RequestAllRefresh()`。

### 草场 Volume 交互

使用 `GameObject > AnimeGrass > GrassVolume` 创建球形或盒形三维区域。位置、旋转和尺寸全部由对象的 `Transform` 控制，`Scale X/Y/Z` 分别对应 Volume 的本地三轴尺寸，三轴旋转均会作用于实际遮罩范围。

- `排除草强度`：`0` 不移除，`1` 完全移除 Volume 内的草。
- `实时跟随 Transform`：通过实时 GPU Volume 排除真实草和远景覆盖，不写入静态地表缓存，适合角色、载具和移动相机。Volume 位于 Camera 或其子对象上时会自动启用实时模式。
- `推动草叶上部`：实时 GPU 顶点效果，不需要重建缓存。通过推动距离、边缘过渡和受影响起始高度控制形变，草根始终保持固定。
- 最多同时处理 16 个启用实时排除或推动的 Volume。静态道路可关闭实时跟随并把排除结果写入地表缓存；移动物体应开启实时跟随。

选中 Volume 时，Scene 视图始终显示其范围；开启“未选中时也显示”后，取消选择仍会保留轮廓。外轮廓表示完整作用边界，红色内轮廓表示排除草完全生效区，青色内轮廓表示草叶推动达到完整强度的区域。轮廓由 Scene Handles 覆盖绘制并穿透草叶显示；选中时的淡色体积填充仍依赖 Scene 视图的 Gizmos 开关。

静态模式下，只有缓存地表点实际位于 Volume 内时才写入排除遮罩，因此悬在上方或位于下方的 Volume 不会误伤当前缓存地表。实时模式直接根据当前世界坐标和三维范围过滤真实草与远景覆盖，并在每个相机渲染前上传最新 Transform，不依赖 `AnimeSurfaceCache` 的更新模式。多个 Volume 和来源遮罩使用最大值合并。

草 Shader 中相关参数包括：

- `地表颜色影响` 与 `仅影响草根`。
- `地表法线影响`。
- `排除遮罩影响` 与 `高度匹配容差`。

`高度匹配容差` 用于多层场景。只有缓存中的最高表面高度与草根高度足够接近时，草才会读取该表面的颜色、法线和遮罩，因此上层平台不会错误影响下层草。需要更严格地区分楼层时减小该值。

其他自定义 Shader 可以包含 `Packages/com.ming.animegrass/Shaders/AnimeSurfaceCache.hlsl`，使用 `AnimeSurfaceCacheWorldToUV`、`AnimeSurfaceCacheContainsUV` 和 `SampleAnimeSurfaceCache` 读取同一份数据。

1024 分辨率缓存的显存占用约为 25 MiB，包含颜色、半精度法线/高度、遮罩、深度和 Mipmap。实际值会随平台支持的 RenderTexture 格式变化。固定缓存不会进行虚拟分页，超大世界应等待后续 Clipmap 或完整 RVT。

## 未来工作：完整 RVT

计划为大型场景增加完整的 Runtime Virtual Texture（RVT）地表缓存系统。目标是在相对固定的显存预算下覆盖更大的世界，并让近处地表保持较高纹素密度。

计划能力包括：

- 虚拟页表与固定大小的物理页面缓存。
- 基于摄像机反馈的页面请求和 GPU 按需生成。
- 页面 LRU 淘汰、缺页回退和缓存占用诊断。
- 局部脏区域失效，支持道路、脚印、湿度、积雪和烧焦等动态变化。
- 页面边界扩展与 Mipmap 生成，降低页面接缝和远距离闪烁。
- 缓存地表颜色、法线、高度、粗糙度、地表类型和交互遮罩。
- 让地形、草、岩石、道路和其他场景物体共享世界空间地表数据。
- 支持草继承地表颜色、草根融合、道路避让、水边变色和动态压草。

完整 RVT 主要面向数公里级开放世界、复杂地形材质和大量动态地表效果。对于中小型场景，将优先保留成本更低的固定地表属性缓存或多级 Clipmap 方案。

RVT 只负责地表材质数据的生成、缓存和共享，不替代现有草实例的 LOD、视锥剔除、距离剔除与 GPU Instancing。
